	...
	if (isHit)
	{
		if (OutHit.bBlockingHit)
		{
			if (GEngine)
			{
				...
				//if (OutHit.GetActor()->GetClass()->ImplementsInterface(UObject_Interface::StaticClass()))
				//{
					//UE_LOG(LogTemp, Warning, TEXT("Testowy tekst, mamy interfejs."));
					//IObject_Interface* ReactingObject =
					//IObject_Interface* InteractInterface = Cast<IObject_Interface>(OutHit);
					//IObject_Interface::Execute_InteractWithObject(OutHit.GetActor());
				//}
				if (OutHit.GetActor() && OutHit.GetActor()->Implements<UObject_Interface>())
				{
					InteractableObject = OutHit.GetActor();
				}
				else
				{
...
void AFPSGameCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Action", IE_Released, this, &AFPSGameCharacter::OnAction);
...
void AFPSGameCharacter::ServerFire_Implementation()
{
...
bool AFPSGameCharacter::ServerFire_Validate()
{
	return true;
}

void AFPSGameCharacter::OnAction()
{
	ServerOnAction();
}

void AFPSGameCharacter::ServerOnAction_Implementation()
{
	UE_LOG(LogTemp, Warning, TEXT("Click E"));

	if (InteractableObject != nullptr)
	{
		IObject_Interface::Execute_InteractWithObject(InteractableObject);
	}
}

bool AFPSGameCharacter::ServerOnAction_Validate()
{
	return true;
}
